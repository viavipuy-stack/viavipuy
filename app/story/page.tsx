...
export default function StoryPage() {
  ...
  const [captionJson, setCaptionJson] = useState<CaptionJson>(getDefaultCaptionJson());
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) { setAuthError("Inicia sesion para acceder."); setLoading(false); return; }
      setUserId(user.id);
      ...
      const { data: story } = await supabase
        ...
        .select("id, media_url, media_type, caption, caption_json, created_at, expires_at")
        .eq("user_id", user.id)
        .maybeSingle();
  ...
  async function handleSaveCaption() {
    ...
    if (error) {
      ...
      if (activeStory) {
        setActiveStory({ ...activeStory, caption: captionJson.line1 || null, caption_json: captionJson });
      }
    }
    ...
    return (
      <main className="vv-form-page">
        <div className="vv-form-container">
          <div className="vv-form-header"><h1 className="vv-form-title">Historia Destacada</h1></div>
          <div className="vv-form-error-box">{authError}</div>
          <a href="/login" className="vv-form-link">Ir a Login</a>
        </div>
      </main>
    );
  }
      ...
      <main className="vv-form-page">
        <div className="vv-form-container">
          <div className="vv-form-header">
            ...
            <p className="vv-form-subtitle">Exclusivo para Diamante</p>
          </div>
          <div className="vv-upsell-card" data-testid="upsell-story">
            ...
            <h3 className="vv-upsell-title">Subi tu historia con Diamante</h3>
            <p className="vv-upsell-text">
              Con el plan Diamante podes subir una historia destacada que se muestra durante 24 horas en la seccion Destacadas, visible para todos los visitantes.
            </p>
            ...
            <a href="/planes" className="vv-btn" style={{ display: "block", textAlign: "center", marginTop: 16 }} data-testid="link-planes-story">
              Ver planes
            </a>
          </div>
  ...
  const previewTime = activeStory ? formatTimeAgo(activeStory.created_at) : "Ahora";
  const previewLine2 = captionJson.line2 ? `${captionJson.line2} · ${previewTime}` : previewTime;
  ...
  return (
    <main className="vv-form-page">
      <div className="vv-form-container">
        <div className="vv-form-header">
          <h1 className="vv-form-title">Historia Destacada</h1>
          <p className="vv-form-subtitle">Tu historia se muestra 24h en Destacadas</p>
        </div>

        {msg && (
          <div className={msg.type === "error" ? "vv-form-error-box" : "vv-form-success-box"} style={{ fontSize: "13px", marginBottom: 14 }}>
            {msg.text}
          </div>
        )}
        ...
        {activeStory && (
          <div className="vv-cuenta-section" data-testid="active-story-preview">
            ...
            <div style={{ borderRadius: 10, overflow: "hidden", marginBottom: 12, background: "#111", position: "relative", aspectRatio: "9/16", maxHeight: 400, display: "flex", alignItems: "center", justifyContent: "center" }}>
              {activeStory.media_type === "video" ? (
                <video src={fixStorageUrl(activeStory.media_url)} controls playsInline style={{ width: "100%", height: "100%", objectFit: "contain" }} />
              ) : (
                <img
              ...
              <div style={{
                ...
                {captionJson.line1.trim() && (
                  <div style={{ fontSize: 14, fontWeight: 600, color: captionJson.color1, lineHeight: 1.35, wordBreak: "break-word", marginBottom: 2 }}>
                    {captionJson.line1}
                  </div>
                )}
                <div style={{ fontSize: 12, color: captionJson.color2, lineHeight: 1.3, opacity: 0.9 }}>
                  {previewLine2}
                </div>
              </div>
            </div>
            ...
            <button
              ...
              style={{ width: "100%", marginTop: 10, color: "#e74c3c", borderColor: "#e74c3c33" }}
              disabled={deleting}
        ...
        <div className="vv-cuenta-section">
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <input
              ...
              onChange={(e) => setCaptionJson({ ...captionJson, line1: e.target.value.slice(0, 100) })}
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <input
              ...
              onChange={(e) => setCaptionJson({ ...captionJson, line2: e.target.value.slice(0, 100) })}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
            <div>
              ...
              <select
                ...
                onChange={(e) => setCaptionJson({ ...captionJson, color1: e.target.value })}
                style={selectStyle}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setCaptionJson({ ...captionJson, color2: e.target.value })}
                style={selectStyle}
          ...
          {activeStory && (
            <button
              type="button"
              className="vv-btn"
              style={{ width: "100%", marginBottom: 12, background: "#222", border: "1px solid #444", color: "#fff" }}
        ...
        <div className="vv-cuenta-section">
          <h2 className="vv-cuenta-label">{activeStory ? "Reemplazar media" : "Subir historia"}</h2>
          ...
          >
            {uploading ? "Subiendo..." : activeStory ? "Seleccionar y reemplazar" : "Seleccionar archivo"}
          </button>
          <p style={{ fontSize: 12, color: "var(--text-secondary, #777)", marginTop: 8, textAlign: "center" }}>
