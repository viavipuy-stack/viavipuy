  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: profRows, error: profErr } = await supabase
        .from("profiles")
        .select(
          "verification_status, plan_actual, plan_estado, paid_until, trial_ends_at, verified_at, categoria, nombre, doc_frente_url, doc_dorso_url, selfie_url, instagram_url, onlyfans_url, twitter_url, email_confirmed, telefono_whatsapp, telegram_username, telefono_visible, video_disponible",
        )
        .eq("id", user.id)
      ...
      const { data: pubData } = await supabase
        .from("publicaciones")
        .select(
          "id, nombre, edad, descripcion, cover_url, zona, ciudad, disponible, rating, telefono, fotos, fotos_preview, videos, video_preview_url, tarifas, consultar_precio, servicios, fantasias, servicios_virtuales, tipos_masajes, idiomas",
        )
        .eq("user_id", user.id)
        .maybeSingle();
          ...
          <div className="vv-cuenta-plan-card">
            ...
            {planFin && (
              <div className="vv-cuenta-row" style={{ marginTop: 10 }}>
                <span className="vv-cuenta-key">
                  {isExpired ? "Vencio el" : "Vence el"}
                </span>
                <span className="vv-cuenta-val">
                  {planFin.toLocaleDateString()}
                </span>
              </div>
            )}
            ...
            <div className="vv-cuenta-plan-limits">
              <span>
                Fotos:{" "}
                {planConfig.max_fotos === null
                  ? "Ilimitadas"
                  : `hasta ${planConfig.max_fotos}`}
              </span>
              <span>
                Videos:{" "}
                {planConfig.max_videos >= 999
                  ? "Ilimitados"
                  : planConfig.max_videos}
              </span>
              <span>
                Metricas:{" "}
