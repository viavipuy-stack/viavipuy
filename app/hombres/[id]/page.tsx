...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/hombres/${id}` },
  };
}

export default async function HombresIdPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  if (UUID_RE.test(id)) {
    return (
      <PerfilView category="hombres">
        <SeoLocationsBlock basePath="/hombres" categoria="hombre" />
      </PerfilView>
    );
  }

  const zona = slugToName(id);
  const { items, count, error } = await fetchPublicacionesByZona(id, "hombre");
  ...
  return (
    <main>
      ...
      {error ? (
        ...
        <ListadoGrid items={items} basePath="/hombres" />
      ) : (
        <div className="vv-zona-empty">
      ...
      {count <= 6 && <SeoLocationsBlock basePath="/hombres" categoria="hombre" />}
